...
from config.task_config import TASK_ID, TASK_IDE, ARTICLE_ACC_TIME, \
    VIDEO_ACC_TIME, DAILY_ANSWER_ACC_TIME, WEEKLY_ANSWER_ACC_TIME, \
    PROJECT_ANSWER_ACC_TIME, TASK_OPTIONS
...
# 输出每周答题任务进度
def out_weekly_answer_bar():
    msg = '''每周答题任务:{play}/{max_task}\n'''
    msg = msg.format(
        play=user_msg.USER_WEEKLY_ANSWER_PLAYING,
        max_task=user_msg.USER_WEEKLY_ANSWER_TASKS
    )
    time_msg = '随机等待时间:{time_sleep}秒'
    if user_msg.USER_WEEKLY_ANSWER_TIME_SLEEP:
        if user_msg.USER_WEEKLY_ANSWER_TIME_SLEEP == WEEKLY_ANSWER_ACC_TIME:
            time_msg = "每周答题任务已完成"
        else:
            time_msg = time_msg.format(
                time_sleep=user_msg.USER_WEEKLY_ANSWER_TIME_SLEEP
            )
        msg += time_msg
    print(msg)
...
def out_project_answer_bar():
    ...
    msg = msg.format(
        play=user_msg.USER_PROJECT_ANSWER_PLAYING,
        max_task=user_msg.USER_PROJECT_ANSWER_TASKS
    )
    time_msg = '随机等待时间:{time_sleep}秒'
    if user_msg.USER_PROJECT_ANSWER_TIME_SLEEP:
        if user_msg.USER_PROJECT_ANSWER_TIME_SLEEP == PROJECT_ANSWER_ACC_TIME:
            time_msg = "专项答题任务已完成"
        else:
            time_msg = time_msg.format(
                time_sleep=user_msg.USER_PROJECT_ANSWER_TIME_SLEEP
            )
        msg += time_msg
    print(msg)
...
def out_print(func):
    def wrapper(*args, **kwargs):
        ...
        if TASK_OPTIONS[4][1]:
            out_weekly_answer_bar()
        if TASK_OPTIONS[5][1]:
            out_project_answer_bar()
